...
from .constraints import ConstraintHandler, ConstraintLevel, ConstraintTime
...
class LSystemModule:
    ...
    def apply_rules(self,
        ...
        logging.getLogger('base-logger').debug(f'[{self.name}] Converting HL axioms to ML...')
...
class LSystem:
    ...
    def __init__(self, hl_solver: LSolver, ll_solver: LSolver, names: List[str]):
        ...
        self.all_hl_constraints = []
        self.all_ll_constraints = []
    ...
    def add_hl_constraints(self, cs: List[List[Optional[ConstraintHandler]]]) -> None:
        ...
        for m, mcs in zip(self.modules, cs):
            for c in mcs:
                ...
                self.all_hl_constraints.append(c)

    # add own constraints as well
    def add_ll_constraints(self, cs: List[List[Optional[ConstraintHandler]]]) -> None:
        assert len(cs) == len(self.modules), f'Wrong number of expected modules: have {len(self.modules)}, passed {len(cs)}.'
        for m, mcs in zip(self.modules, cs):
            for c in mcs:
                m.add_ll_constraint(c)
                self.all_ll_constraints.append(c)

    def process_module(self,
                       module: LSystemModule,
                       starting_axiom: str,
                       iterations: int = 1) -> Tuple[List[Structure], List[str], List[str]]:
    ...
    def _produce_axioms_combinations(self,
        ...
        return [self._merge_axioms(x) for x in list(itertools.product(*m_axioms))]

    def apply_rules(self,
                    starting_axioms: List[str],
                    iterations: List[int],
                    make_graph: bool = False) -> Tuple[List[Structure], List[str], List[str]]:
        assert len(starting_axioms) == len(self.modules), f'Assumed wrong number of modules: have {len(self.modules)}, passed {len(starting_axioms)}.'
        assert len(iterations) == len(self.modules), f'Assumed wrong number of modules: have {len(self.modules)}, passed {len(iterations)}.'

        all_axioms = []
        for module, starting_axiom, n_iterations in zip(self.modules, starting_axioms, iterations):
            _, hl_axioms, _ = self.process_module(module=module, starting_axiom=starting_axiom, iterations=n_iterations)
            all_axioms.append(hl_axioms)

        all_axioms = self._produce_axioms_combinations(m_axioms=all_axioms)
        ml_axioms = [self.hl_solver.translator.transform(axiom=hl_axiom) for hl_axiom in all_axioms]
        ll_axioms = [self.ll_solver.solve(axiom=ml_axiom,
                                          iterations=1,
                                          axioms_per_iteration=1,
